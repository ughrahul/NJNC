...
export default async function adminRoutes(app: FastifyInstance) {
  ...
  app.get("/stats", async (_req: FastifyRequest, reply: FastifyReply) => {
    const [
      ...
      app.prisma.abstract.count({
        where: { status: "SUBMITTED", deletedAt: null },
      }),
      app.prisma.abstract.count({
        where: { status: { in: ["ACCEPTED", "REJECTED"] }, deletedAt: null },
      }),
    ...
    return reply.send(
      successResponse({
        ...
        payments: {
          ...
          byStatus: paymentBreakdown.map((p) => ({
            status: p.status,
            count: p._count,
          })),
        },
  ...
  app.get(
    ...
    async (_req: FastifyRequest, reply: FastifyReply) => {
      ...
      // Build CSV
      const headers =
        "Registration Code,Name,Email,Country,Institution,Specialty,Category,Payment Status,Created At\n";
  ...
  app.get(
    ...
    async (_req: FastifyRequest, reply: FastifyReply) => {
      ...
      const headers =
        "Title,Author,Email,Institution,Topic,Type,Status,Score,Created At\n";
