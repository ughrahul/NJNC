...
export default async function contentRoutes(app: FastifyInstance) {
  ...
  app.get('/:section', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    const content = await app.prisma.contentBlock.findUnique({ where: { section } });
  ...
  app.put('/:section', { preHandler: [extractUser, requireAuth, requireRole('ADMIN')] }, async (req: FastifyRequest, reply: FastifyReply) => {
    const { section } = req.params as { section: string };
  ...
  app.get('/news/list', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    return reply.send(successResponse(posts, { page, perPage, total, totalPages: Math.ceil(total / perPage) }));
  });
