...
export default async function speakerRoutes(app: FastifyInstance) {
  ...
  app.get("/:id", async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!speaker || !speaker.isPublished)
  ...
  app.post(
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      ...
      const speaker = await app.prisma.speaker.create({
        data: { ...parsed.data, updateToken },
      });
  ...
  app.put(
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      ...
      if (!parsed.success) throw new ValidationError("Validation failed");
      const speaker = await app.prisma.speaker.update({
        where: { id },
        data: parsed.data,
      });
      return reply.send(successResponse(speaker));
  ...
  app.put(
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      ...
      const parsed = updateSpeakerSchema.safeParse(req.body);
      if (!parsed.success) throw new ValidationError("Validation failed");
      const updated = await app.prisma.speaker.update({
        where: { id: speaker.id },
        data: parsed.data,
      });
