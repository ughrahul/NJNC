...
export default async function abstractRoutes(app: FastifyInstance) {
  ...
  app.get(
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      ...
      const { abstracts, total } = await service.list(
        pagination.page,
        pagination.perPage,
        {
          status: query.status,
  ...
  app.get(
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      const { id } = idParamSchema.parse(req.params);
      const abstract = await service.getById(
        id,
        req.user!.userId,
        req.user!.role,
      );
  ...
  app.patch(
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      ...
      if (!parsed.success)
        throw new ValidationError("Provide 2-3 reviewer IDs");
  ...
  app.patch(
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      ...
      if (!parsed.success) throw new ValidationError("Validation failed");
      const abstract = await service.makeDecision(
        id,
        parsed.data.status,
        parsed.data.reviewerComments,
      );
      return reply.send(successResponse(abstract));
    },
  );
