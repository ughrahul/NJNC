...
import { uploadPaymentProofSchema, verifyPaymentSchema } from '../schemas/payment.schema';
...
export default async function paymentRoutes(app: FastifyInstance) {
  ...
  app.post('/upload-proof', { preHandler: [requireAuth] }, async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!registration) throw new NotFoundError('Registration', parsed.data.registrationId);
  ...
  app.get('/', { preHandler: [requireAuth, requireRole('ADMIN')] }, async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    return reply.send(successResponse(payments, paginationMeta(pagination.page, pagination.perPage, total)));
  });

  // PATCH /api/payments/:id/verify (admin) — Verify or reject payment
  app.patch('/:id/verify', { preHandler: [requireAuth, requireRole('ADMIN')] }, async (req: FastifyRequest, reply: FastifyReply) => {
