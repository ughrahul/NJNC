...
const tc: Record<string, string> = {
  KEYNOTE: "bg-crimson-50 text-crimson-700",
  PANEL: "bg-primary-50 text-primary-700",
  WORKSHOP: "bg-forest-50 text-forest-700",
};
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button className="px-4 py-2 bg-forest text-white rounded-lg text-sm font-semibold hover:bg-forest-700 transition-colors">
          Publish All
        </button>
      </div>
                ...
                {sessions
                  ...
                  .map((s) => (
                    <tr key={s.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-3">
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full ${tc[s.type] || "bg-gray-100"}`}
                        >
                          {s.type}
                        </span>
                      </td>
                      <td className="p-3">
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full ${s.published ? "bg-forest-50 text-forest-700" : "bg-gray-100 text-gray-600"}`}
                        >
      ...
      <button className="px-4 py-2 border border-dashed border-gray-300 rounded-lg text-sm text-slate hover:border-primary hover:text-primary transition-colors w-full">
        + Add Session
      </button>
    </div>
  );
