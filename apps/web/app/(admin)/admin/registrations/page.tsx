  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button className="px-4 py-2 bg-primary text-white rounded-lg text-sm font-semibold hover:bg-primary-700 transition-colors">
          Export CSV
        </button>
      </div>
      ...
      <div className="flex gap-2">
        {["All", "PAID", "PENDING", "CANCELLED"].map((s) => (
          <button
            key={s}
            onClick={() => setFilter(s)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${filter === s ? "bg-primary text-white" : "bg-white text-slate border hover:border-primary"}`}
          >
            {s}
          </button>
        ))}
          ...
          <tbody>
            {filtered.map((r) => (
              <tr key={r.id} className="border-b hover:bg-gray-50">
                ...
                <td className="p-4">
                  <span
                    className={`text-xs px-2.5 py-1 rounded-full font-medium ${statusColors[r.status] || ""}`}
                  >
                    {r.status}
                  </span>
                </td>
                <td className="p-4">
                  <div className="flex gap-2">
                    {r.status === "PENDING" && (
                      <button className="text-xs px-3 py-1 bg-forest text-white rounded-lg hover:bg-forest-700">
                        Verify
                      </button>
                    )}
